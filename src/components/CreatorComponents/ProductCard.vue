<template lang="pug">
.product-card()
  v-row.ma-0(style="width:100%; align-content:center; padding: 16px 0 16px 0" @click="selectProduct(productIndex)")
    v-col.d-flex.align-self-center(cols='1')
    ...
    v-col.d-flex.align-self-center.pl-2(cols='8')
      p.mb-0.product-text-title {{productData.title}}
    v-col.d-flex.align-self-center.text-right(cols='3')
      p.mb-0.product-text-title {{productData.currency}}{{productData.rate}}
  v-row.ma-0(style="width:100%;")
    p.mb-0.product-text-desc {{productData.description}}
  v-row.ma-0(style="width:100%; padding-bottom: 16px")
    v-btn.product-chooose-btn()
      | Choose
</template>
...
export default {
  ...
  methods: {
    selectProduct(index) {
      ...
      const selectedCard = allCard[index]
