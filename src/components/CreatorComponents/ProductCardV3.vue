...
.product-card
  v-expansion-panels
    v-expansion-panel.mb-2.product-card-main(v-for="(product, index) in products" :key="product.product_id")
        ...
        v-row.ma-0.my-row()
          //- .qty-picker
            v-row(style="height:100%")
              ...
              div
                div.number-box
                  p.mb-0 {{selected[index].qty}}
              eva-icon.d-flex.align-self-center.qty-btn(name="plus-circle" height="20px" width="20px" fill="#231F20" @click="changeValue('inc',index)")
...
export default {
  ...
  mounted() {
    this.products.forEach(prod => {
      const i = { id: prod.product_id, title: prod.title, qty: 0}
  ...
  methods: {
    ...
    changeValue(type,index) {
      ...
      console.log(item);
      if (type === 'inc') {
        if(item.qty < this.maxValue)
          this.selected[index].qty++
      }
      if (type === 'dec') {
        if(item.qty > this.minValue)
          this.selected[index].qty--
      }
    }
  }
